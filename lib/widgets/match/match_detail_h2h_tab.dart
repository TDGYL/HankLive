...
class _MatchDetailH2HTabState extends State<MatchDetailH2HTab> {
  ...
  List<HankH2HMatch> get _filteredMatches {
    ...
    if (_leagueOnly) {
      // mock数据中 competitionName 为"西甲"的视为联赛
      result = result.where((m) => m.competitionName == '西甲').toList();
    }
  ...
  Widget _buildWDLSummary(List<HankH2HMatch> displayMatches) {
    ...
    final avgGoals = total > 0 ? ((homeGoals + awayGoals) / total).toStringAsFixed(1) : '0.0';
                      ...
                      Row(
                        children: [
                          if (m.competitionLogo.isNotEmpty)
                            ...
                            const Icon(Icons.emoji_events, size: 14, color: AppColors.violet400),
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: Row(
                          children: [
                            if (m.homeTeamLogo.isNotEmpty)
                              ClipRRect(
                                borderRadius: BorderRadius.circular(4),
                                child: Image.network(m.homeTeamLogo,
                                    width: 24,
                                    height: 24,
                                    fit: BoxFit.cover,
                                    errorBuilder: (_, __, ___) => const SizedBox()),
                      ...
                      Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            const Text('-', style: TextStyle(fontSize: 12, color: AppColors.slate400)),
                            const SizedBox(width: 4),
                            Text(
                              '${m.awayNormalScore ?? 0}',
                              style: TextStyle(
                      ...
                      Expanded(
                        child: Row(
                          ...
                          children: [
                            ...
                            const SizedBox(width: 8),
                            if (m.awayTeamLogo.isNotEmpty)
                              ClipRRect(
                                borderRadius: BorderRadius.circular(4),
                                child: Image.network(m.awayTeamLogo,
                                    width: 24,
                                    height: 24,
                                    fit: BoxFit.cover,
                                    errorBuilder: (_, __, ___) => const SizedBox()),
